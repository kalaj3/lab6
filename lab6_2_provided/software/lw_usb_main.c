...
{
	BYTE transmit_buf[2] = {reg + 2, val};
	BYTE receive_buf[2] = {0, 0};

	Status = XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Slave Select L 1");
		while (1)
		{
			;
		}
	...
	Status = XSpi_Transfer(&SpiInstance, transmit_buf, receive_buf, 2);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Transfer L 1");
		while (1)
		{
			;
		}
	...
	BYTE return_code = receive_buf[0];

	if (return_code != 0)
	{
		xil_printf("MAXreg_wr Error: Return code %d\n", return_code);
	}

	Status = XSpi_SetSlaveSelect(&SpiInstance, 0x00);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Slave Select L 2");
		while (1)
		{
			;
		}
...
BYTE *MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE *data)
{
	...
	BYTE transmit_buf[1 + nbytes];
	BYTE receive_buf[1 + nbytes];

	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	transmit_buf[0] = reg + 2;
	memcpy(&transmit_buf[1], data, nbytes);
	memset(receive_buf, 0, sizeof(receive_buf)); // clear reciever

	XSpi_Transfer(&SpiInstance, transmit_buf, receive_buf, 1 + nbytes);

	XSpi_SetSlaveSelect(&SpiInstance, 0x00);

	BYTE return_code = receive_buf[0]; //* mosi sends {command,data} and miso sends {return code, value}
									   //? how does spi understand read vs write? does it send/return dif or same structures?
									   //? might be in the reg we send it but that seems suss
	if (return_code != 0)
	{
		xil_printf("MAXreg_wr Error: Return code %d\n", return_code);
		return return_code; // error = return error
	}

	return (data + nbytes); // ptr to next byte
							// psuedocode:
...
BYTE MAXreg_rd(BYTE reg)
{
	...
	XSpi_Transfer(&SpiInstance, transmit_buf, receive_buf, 2); // force 2

	XSpi_SetSlaveSelect(&SpiInstance, 0x00); // disconnect

	BYTE return_val = receive_buf[1];

	BYTE return_code = receive_buf[0]; //* mosi sends {command, data/junk} and miso sends {return code, value}
...
BYTE *MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE *data)
{
	...
	XSpi_Transfer(&SpiInstance, transmit_buf, receive_buf, 2 + nbytes); //*simul transmition, takes 2 bytes for transmit, remaining for return and overlaps 1
	...
	if (return_code != 0)
	{
		xil_printf("MAXbytes_rd Error: Return code %d\n", return_code);
		return NULL; // Return NULL on error
	}

	memcpy(data, &receive_buf[1], nbytes);
